...
class TaskService(BaseService):
    ...
    @transaction_mode
    async def get_user_task_stats(self, user_id: UUID) -> dict[str, int]:
        return {
            'assigned_tasks_count': await self.uow.task.count_assigned_tasks(user_id),
            'watched_tasks_count': await self.uow.task.count_watched_tasks(user_id),
        }
